...
import { expertise, projects, talks, writing } from './data/portfolio'

const nav = [['work', 'Work'], ['writing', 'Writing'], ['talks', 'Talks'], ['creative', 'Creative']]
        ...
        <section id="top" className="hero shell">
          <div className="hero-copy">
            ...
            <div className="actions">
              ...
              <a className="button ghost" href="https://github.com/sibStephen" target="_blank" rel="me noopener noreferrer"><Github size={18} /> GitHub</a>
            </div>
          </div>
          <div className="terminal-card" aria-label="Technology profile summary">
            <div className="terminal-bar"><span/><span/><span/><p>profile.json</p></div>
            <pre><code>{`{
  "name": "Sibu Stephen",
  "focus": [
    "Drupal architecture",
  ...
  "status": "always learning"
        ...
        <section id="work" className="section shell">
          ...
          <div className="project-grid">
            {filteredProjects.map(project => <ProjectCard key={project.url} project={project} />)}
          </div>
            ...
            <div className="writing-grid">
              {writing.map((item, index) => (
                <a className="writing-card" href={item.url} target="_blank" rel="me noopener noreferrer" key={item.url}>
                  ...
                  <div><p className="label">{item.label}</p><h3>{item.title}</h3><p>{item.description}</p></div>
        ...
        <section id="talks" className="section shell">
          <div className="section-heading"><p className="eyebrow">03 · Speaking</p><h2>Talks, camps and interviews</h2></div>
          <div className="talk-layout">
            ...
            <div className="talk-list">
              {talks.map((talk, index) => (
                <a href={talk.url} target="_blank" rel="noopener noreferrer" key={talk.url}>
                  <span>{String(index + 1).padStart(2, '0')}</span><div><h3>{talk.title}</h3><p>{talk.detail}</p></div><ArrowUpRight size={18}/>
                </a>
              ))}
            </div>
          </div>
        ...
        <section id="creative" className="section shell">
          <div className="creative-card">
            <div><p className="eyebrow">04 · Beyond code</p><h2>Technology is one part of the story.</h2><p>Music, public speaking and writing keep creativity at the center of how I think and build.</p></div>
            <div className="creative-links">
              ...
              <a href="https://git.drupalcode.org/project/intentui" target="_blank" rel="noopener noreferrer"><Code2/><span><b>Open source</b>Drupal contribution</span><ArrowUpRight/></a>
            </div>
          </div>
        </section>
      </main>
      ...
      <footer className="site-footer">
        <div className="shell footer-wrap"><div><a className="brand" href="#top"><span>&lt;</span>SS<span>/&gt;</span></a><p>Designed and built with React, accessibility and curiosity.</p></div><div className="footer-links"><a href="/llms.txt">AI profile</a><a href="/.well-known/ai-catalog.json">AI catalog</a><a href="/profile.json">Profile JSON</a></div></div>
      </footer>
    </>
  )
}
